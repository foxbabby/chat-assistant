...
def find_wechat_window(previous_wid: int | None = None) -> WindowInfo | None:
    ...
    import AppKit
    apps = AppKit.NSRunningApplication.runningApplicationsWithBundleIdentifier_("com.tencent.xinWeChat")
    ...
    for w in wins:
        ...
        if int(w.get("kCGWindowOwnerPID") or 0) not in pids:
            continue
...
def _vision_blocks(handler, languages, chat_only: bool) -> list[TextBlock]:
    ...
    success, request_error = handler.performRequests_error_([req], None)
    if request_error:
        errors.append(str(request_error))
    if errors or not success:
        raise ValueError('系统文字识别失败：' + ('; '.join(errors) if errors else 'Vision 请求未完成'))
...
def ocr(path: Path, languages=("zh-Hans",), chat_only: bool = True) -> list[TextBlock]:
    ...
    if image is None:
        raise ValueError('无法读取微信截图')
    return ocr_image(image, languages, chat_only)
...
def _cached_blocks(key, recognize):
    # Bound memory use; never return mutable OCR blocks owned by the cache.
    with _ocr_lock:
        now = time.monotonic()
        cached = _ocr_cache.get(key) if key is not None else None
        ...
        if key is not None:
            ...
            _ocr_cache[key] = (now, copy.deepcopy(blocks))
        return blocks


def ocr_image(image, languages=("zh-Hans",), chat_only: bool = True) -> list[TextBlock]:
    """Recognize changed pixels locally; reuse unchanged panes for up to 30 seconds."""
    import Vision
    key = None
    try:
        w, h = Quartz.CGImageGetWidth(image), Quartz.CGImageGetHeight(image)
        region = (Quartz.CGImageCreateWithImageInRect(image, Quartz.CGRectMake(
            int(CHAT_PANE_X_MIN * w), 0, w - int(CHAT_PANE_X_MIN * w),
            int((1 - INPUT_AREA_Y_MIN) * h))) if chat_only else image)
        pixels = Quartz.CGDataProviderCopyData(Quartz.CGImageGetDataProvider(region))
        key = (w, h, tuple(languages), chat_only, hashlib.sha256(bytes(pixels)).digest())
    except Exception:
        pass  # A failed change detector must never suppress a fresh read.
    def recognize():
        handler = Vision.VNImageRequestHandler.alloc().initWithCGImage_options_(image, None)
        return _vision_blocks(handler, languages, chat_only)
    return _cached_blocks(key, recognize)
...
def extract_chat_title(blocks: list[TextBlock]) -> str:
    ...
    cands = [b for b in blocks
             ...
             and b.conf >= 0.85 and len(b.text) >= 1 and not _is_noise(b)
             and re.search(r"[\w\u4e00-\u9fff]", b.text)]
    if not cands:
        return ""
...
def read_conversation(max_messages: int = 12, previous_wid: int | None = None,
    ...
    # A non-null CGImage may still be blank on newer macOS or sleeping displays.
    # Do not treat that as a successful capture; try the system capture utility once.
    if image is not None and not blocks:
        with tempfile.TemporaryDirectory() as td:
            png = Path(td) / 'wechat-fallback.png'
            if capture_window(win.wid, png):
                blocks = ocr(png)
                capture_path = 'subprocess-empty-frame-fallback'
                t_ocr = time.perf_counter()
    if not blocks:
        return {'ok': False, 'error': 'empty_capture', 'messages': [],
                'window': window, 'n_blocks': 0}
