...
def accessibility_diagnostics() -> dict:
    result = {}
    try:
        result['trusted'] = bool(ApplicationServices.AXIsProcessTrusted())
    except Exception as error:
        result['trusted'] = False
        result['check_error'] = type(error).__name__ + ': ' + str(error)
    app = _wechat_app()
    if app is not None:
        try:
            element = ApplicationServices.AXUIElementCreateApplication(app.processIdentifier())
            err, windows = ApplicationServices.AXUIElementCopyAttributeValue(
                element, ApplicationServices.kAXWindowsAttribute, None)
            result['wechat_ax_error'] = int(err)
            result['wechat_windows_readable'] = err == 0
        except Exception as error:
...
def _find_input_box(pid: int, expected_window=None):
    ...
    for window in ordered:
        if expected_window:
            from ax_reader import rect
            frame=rect(window)
            if not frame or any(abs(frame[i]-expected_window[key])>4 for i,key in enumerate(('x','y','w','h')) if key in expected_window):
                continue
        queue, seen = [window], 0
